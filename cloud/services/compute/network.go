...
func (s *Service) createSubnetworks() error {
	for _, subnet := range s.scope.Subnets() {
		...
		if gcperrors.IsNotFound(err) {
			...
			subnet, err := s.subnetworks.Get(s.scope.Project(), s.scope.Region(), subnetSpec.Name).Do()
			if err != nil {
				return errors.Wrapf(err, "failed to describe subnetwork")
			}

			s.scope.GCPCluster.Status.Network.Subnet = pointer.StringPtr(subnet.SelfLink)
