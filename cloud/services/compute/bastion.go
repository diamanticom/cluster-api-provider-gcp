...
func (s *Service) ReconcileBastion() error {
	...
	if instance != nil {
		...
		if len(instance.NetworkInterfaces) > 0 {
			if len(s.scope.GCPCluster.Status.Bastion.Addresses) == 0 {
				s.scope.GCPCluster.Status.Bastion.Addresses = make([]v1.NodeAddress, 1)
			}
			s.scope.GCPCluster.Status.Bastion.Addresses[0].Type = corev1.NodeExternalIP
			s.scope.GCPCluster.Status.Bastion.Addresses[0].Address = instance.NetworkInterfaces[0].AccessConfigs[0].NatIP
		}
	}
...
func (s *Service) getDefaultBastion() *compute.Instance {
	...
	sshkey = fmt.Sprintf("ubuntu:%s", sshkey)
		...
		NetworkInterfaces: []*compute.NetworkInterface{
			{
				...
				Subnetwork: s.scope.SubnetSelfLink(),
